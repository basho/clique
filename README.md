...
* `text` - Represents an `iolist`.
* `column` - A title (`iolist`) and values (a list of `iolist`) intended to be displayed consecutively (**not** as part of a table). Do not assume rendering will be vertical.
* `table` - A matrix of values constructed from a list of proplists. Each proplist represents a row in the table. The keys in the first row represent column headers; each following row (proplist) must contain the same number of taggled tuples but the keys are ignored.
* `alert` - A list of status types representing an error condition.

## Type assembly functions

See descriptions above for the arguments to each.

* `riak_cli_status:text/1`
* `riak_cli_status:column/2`
* `riak_cli_status:table/1`
* `riak_cli_status:alert/1`
